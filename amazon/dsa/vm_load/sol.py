...
def min_vms_required(jobs: list[tuple[int, int]]) -> int:
    ...
    """
        I'll sort jobs by start time so I can process them in the order
        they begin, similar to how we would allocate machines over time
    """
    ...
    for start, end in sorted_jobs:
        if start > end:
            raise ValueError("Job start time cannot be after end time")
        
        """
            If a job has zero duration, I'll treat it as not needing a
            VM because it does not actually run over any time interval
        """
        if start == end:
            continue
        
        """
            Before assigning a VM to this job, I'll free all machines
            whose jobs have already ended by this start time
        """
        while active_end_times and active_end_times[0] < start:
            heapq.heappop(active_end_times)
            
        # Now this job needs a VM, so I'll add its end time to the heap
